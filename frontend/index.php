...
<html lang="en">
  <head>
    ...
    <link href="mazaar/css/bootstrap.css" rel="stylesheet" type="text/css">
    <link href="mazaar/css/theme.css" rel="stylesheet" type="text/css">
    <link href="mazaar/css/skins/skin-default.css" rel="stylesheet" type="text/css">
    <link href="mazaar/css/plugins.css" rel="stylesheet" type="text/css">
...
<body>
  <?php
   ...
   $sitedata = limited_records_where('sites','theme,name','id',$_GET['siteid']);
   if ($sitedata->num_rows > 0) {
   $themeid = mysqli_fetch_object($sitedata);
//    echo $themeid->theme; exit;
	if(isset($_GET['pageid'])){
		$page_data = limited_records_where('pages','folderpath,filename','id',$_GET['pageid']);
		$page_record = mysqli_fetch_object($page_data);
		// echo $page_record->folderpath.'---';
		if($page_record->folderpath !=''){
		$site_folder = $page_record->folderpath.$page_record->filename.'.html';
		$pagename = ucfirst($page_record->filename);
		}else{
			$site_folder = 'demo/elevated-2/about-us.html';
			$pagename = 'about-us';
		}
	  }

	     $module_data = get_records_where('modules','*','themeid', $themeid->theme);
   // print_r($module_data); exit;
   if ($module_data->num_rows > 0) {

     while($row = $module_data->fetch_assoc()) {
       if($row['image'] != ''){
         $iconimage = Module_URL.$row['image'];
       }else{
          $iconimage =  BASE_URL.'libs/builder/icons/fullwidth.png';
       }
              $module_name[] = "bootstrap4/".str_replace(' ','-',strtolower($row['name']));
              $singlemodule = 'Vvveb.Blocks.add("bootstrap4/'.str_replace(' ','-',strtolower($row['name'])).'", {
                              name: "'.$row['name'].'",
                              dragHtml: "<img src='.$iconimage.'>",
                              image: "'.$iconimage.'",
                              html: `'.$row['html'].'`,
                          });';
                          $singlemodule11 .= $singlemodule;
      }
      // exit;
      $html_block .= "[";
      $html_block .= '"' . implode('","', $module_name) . '"';
      $html_block .= "];";
      // echo $html_block; exit;
      $inital = 'Vvveb.BlocksGroup["Pre Built Elements"] ='.$html_block;
      $block_html = $inital . $singlemodule11;

        file_put_contents(__DIR__."/libs/builder/blocks-bootstrap4.js", $block_html);
         //die();
   }else{
     // $stylefile = fopen(__DIR__."/libs/builder/blocks-bootstrap4.js", "w") or die("Unable to open file!");
     $modules = file_get_contents(BASE_URL.'libs/builder/blocks-bootstrap4-backup.js', false);
     file_put_contents(__DIR__."/libs/builder/blocks-bootstrap4.js", $modules);
   }
			...
			<fieldset>
				...
				<div class="panel-section">
					...
					<div class="form-group">
						...
						<input type="text" name="page_name" id="page_nam" placeholder="home page test">
					</div>
					<div class="form-group">
						<label for="Meta Description">Meta Description</label>
						<textarea name="meta_description" id="meta_des" placeholder="text type here"></textarea>
						<span class="characters_count"></span>
					</div>
					<div class="form-group">
