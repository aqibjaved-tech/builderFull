...
// 		$('#save-btn').attr('disabled','disabled');
		...
		if (!startTemplateUrl || startTemplateUrl == null) {
			var body = this.getHtml().split('</head>');
			var content = body[1].toString();
			var split_content = content.split('<div class="end">'); 
		}

		console.log(split_content[0]);
        
        var dataToSend = JSON.stringify(window.btoa(unescape(encodeURIComponent(this.getHtml()))));
		
		$.ajax({
            url: 'save.php',//set your server side save script url
            type: "POST",
            dataType: "json",
            data: {html: dataToSend, pageFileName: data["pageFileName"]},
